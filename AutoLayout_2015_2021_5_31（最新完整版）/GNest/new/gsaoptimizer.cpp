...
int GSAOptimizer::optimize(int maxIter,							// 最大迭代次数
	...
	int *pProgress)
{
	...
	int minFirstPageNestHeight = 1000000;
	int   isSameNum = 0;
	...
	int firstPageNestLength = 0;
	localBestRating = layout(localBestSeq, tmpNestParts, pageNum, plateNum, firstPageNestLength);
	if (minFirstPageNestHeight > firstPageNestLength)
		minFirstPageNestHeight = firstPageNestLength;
	...
	while (t > minTemperature)
	{
		for (int iter = 0; iter < saPerIters && (!mIsAbort); iter++)
		{
			...
			{
				SaveLayoutDatas saveDatas = hashIter.value();

				markRating = saveDatas.nestRating;
				tmpRating = markRating;
			}else
			{
				int firstPageNestLength = 0;
				tmpRating = layout(tmpSeq, tmnestParts, tmusedPageNums, tmplateIndices, firstPageNestLength);
				if (minFirstPageNestHeight > firstPageNestLength)
					minFirstPageNestHeight = firstPageNestLength;
				SaveLayoutDatas saveDatas;
				saveDatas.nestParts = tmnestParts;
				saveDatas.nestRating = tmpRating;
				saveDatas.plateIndices = tmplateIndices;
				saveDatas.usedPageNums = tmusedPageNums;
				saveDatas.firstPageNestHeight = firstPageNestLength;

				mRecordLayoutSeQ.insert(tmpSequence, saveDatas);
			}
			...
			if ((localBestRating > globalBestRating) || mIsAbort)
			{
				globalBestSeq = localBestSeq;
				globalBestRating = localBestRating;
