...
#define e 2.718
#define MAXITERM 30    // 定义萤火虫最大迭代次数
...
double GFireFlyOptimizer::calAttraction_i_j(double rI_J)
{
	...
	if ((attractionResult > 0.37) && (attractionResult < 0.5)) attractionResult *= 1.6;		// 避免萤火虫寻优后期大量相同序列时，零件序列改变较少
...
// 去除sequence中重复的值，并将其设置为-1，将去重后的序列保存到noRepeates中
...
// 执行萤火虫算法
double GFireFlyOptimizer::optimize(int maxIter,			// 最大迭代次数
...
#pragma omp parallel for  num_threads(OMP_NUM_THREADS)
		...
		if (((maxPageRating - gBestRating) > 1e-6) ||(fabs(maxPageRating - gBestRating) < 1e-6 && tmpLastPageForwardLines < gLastPageForwardLines))
		{
			...
			if (mIsAbort || ((gBestRating - maxLayoutRating)>= 1e-6) )  // 达到目标排版率直接退出迭代,获知外界直接强制退出迭代
			{
