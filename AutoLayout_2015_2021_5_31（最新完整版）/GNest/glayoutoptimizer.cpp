...
void GLayoutOptimizer::setClusterGGroups(const QVector<QPair<int, int>>	&clusterGGroups)
{
	mClusterBGroups = clusterGGroups;
}

void GLayoutOptimizer::setClusterSGroups(const QVector<QPair<int, int>>	&clusterSGroups)
{
	mClusterSGroups = clusterSGroups;
}
...
double GLayoutOptimizer::optimize(int maxIter,
...
void  GLayoutOptimizer::layout(QVector<QList<QVector<int> > >  layoutSeq, QVector< GPart > &tmpNestParts,
							   QVector<int> &tmpUsedPageNums, QVector<int> &tmpPlateIndices,
							   double &nestRating,double &height, int &lastForwardLineLen)
{
	...
	gautoLayout2.setGroupIndices(mClusterBGroups);
	gautoLayout2.setSubGroupIndices(mClusterSGroups);
	...
	gautoLayout2.layout(tempSorts2, rotFlags, tmpNestParts, tmpUsedPageNums, tmpPlateIndices, nestRating, lastForwardLineLen, 0);
